...
static image_opencl_t* configure_opencl(unsigned int platform, unsigned int device, image_opencl_t* opencl,
    ...
    if (image_opencl_init(opencl, device_id, width, height) < 0) {
...
int main(int argc, char* argv[]) {
    ...
    if (use_method_serial) {
        //image = image_create("serial", sinoscope_image_serial, width, height, 200.0);
    } else if (use_method_openmp) {
        //image = image_create("openmp", sinoscope_image_openmp, width, height, 200.0);
    } else if (use_method_opencl) {
        image = create_image("opencl", image_opencl, width, height);
    } else {
        LOG_ERROR("no method defined");
        exit(1);
    }

    image->opencl_image = image_opencl_ptr;
    ...
    if (image_opencl_ptr != NULL) {
        image_opencl_cleanup(image_opencl_ptr);
    }
